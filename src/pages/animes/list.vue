    ...
    <z-paging ref="paging" :fixed="false" v-model="list" show-refresher-when-reload :default-page-size="10"
        ...
        <view class="main-wrapper">
            <view class="album-list">
                <AnimeItem :item="item" v-for="(item, index) in list" :key="index" />
            </view>
        </view>
    </z-paging>
</template>
...
import type { CategoryModel, AnimesModel } from '@/api/animes/models';
import { animesApi } from '@/api/animes';

import AnimeItem from '@/components/anime-item.vue';
...
const getCategorys = async () => {
    const { items } = await animesApi.apiCategorys({ category_id: 1 });
...
const getList = async (pageNo?: number, pageSize?: number) => {
    ...
    const { items } = await animesApi.apiAnimes(params);
