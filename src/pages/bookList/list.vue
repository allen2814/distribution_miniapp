    ...
    <z-paging ref="paging" :fixed="false" v-model="list" show-refresher-when-reload :default-page-size="10"
        ...
        <view class="main-wrapper">
            <view class="album-list">
                <BooksItem :item="item" v-for="(item, index) in list" :key="index" />
            </view>
        </view>
    </z-paging>
</template>
...
import type { AlbumCategoryModel, BookListModel } from '@/api/models/index';
import { apiStoryCategorys, apiBookLists } from '@/api';

import BooksItem from '@/components/books-item.vue';
...
const getList = async (pageNo?: number, pageSize?: number) => {
    ...
    const { items } = await apiBookLists(params);
