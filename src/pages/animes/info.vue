		...
		<view class="page-content">
			<view class="album-info-page mt30">
				<view class="album-info">
					...
					<view class="info">
						<text class="info-title l1">{{ info?.anime_name }}</text>
						<text class="info-subtitle">ID:{{ info?.anime_id }}</text>
						<text class="info-subtitle l1">
							{{ info?.theme_name }} ·
							{{ info?.setting_name }} ·
							{{ info?.background_name }}
						</text>
						<text class="info-subtitle">
							{{ info?.serial_status == 1 ? '已完结' : '连载中' }}
						</text>
					</view>
				</view>
				<view class="info-content">
					...
					<view class="info-intro">
						...
						<div class="content-box" v-html="ReplaceNewlinesWithBr(info?.desc ?? '')"></div>
					</view>
				</view>
			</view>
		</view>
...
const info = ref<AnimesModel | null>(null);
...
const handlePromotion = () => {
	uni.navigateTo({
		url: `/pages/animes/promotion?anime_id=${info.value?.anime_id}&anime_name=${info.value?.anime_name}`,
	});
};
...
const getInfo = async () => {
	...
	try {
		const [animeRes, materialRes] = await Promise.all([
			animesApi.apiAnimeInfo(id.value),
			animesApi.apiAnimeMaterial(id.value)
		]);
		info.value = animeRes.items;
