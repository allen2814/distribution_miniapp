    ...
    <z-paging ref="paging" :fixed="false" v-model="list" show-refresher-when-reload :default-page-size="10"
        ...
        <view class="main-wrapper">
            <view class="album-list">
                <AlbumItem :item="item" v-for="(item, index) in list" :key="index" />
            </view>
        </view>
    </z-paging>
</template>
...
import type { AlbumCategoryModel, AlbumModel } from '@/api/models/index';
import { apiAlbumCategorys, apiAlbums } from '@/api';
...
const getCategorys = async () => {
    const { items } = await apiAlbumCategorys();
...
const getList = async (pageNo?: number, pageSize?: number) => {
    ...
    const { items } = await apiAlbums(params);
